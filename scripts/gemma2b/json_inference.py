...
# ------------------------------------------------------------------------
PROMPT_FILE = globals().get("PROMPT_FILE", "data/renellm/jailbreaks/jb400.json")
...
# 2. Load JSON Data
# ------------------------------------------------------------------------
def load_prompts_json(file_path):
    """
    Expects a JSON file that is either:
      1) A list of strings, OR
      2) A list of objects where each object has a 'prompt' key.
    ...
    # 1) If it's a list of strings directly:
    if all(isinstance(item, str) for item in data):
        return data

    # 2) If it's a list of dicts with 'prompt' keys:
    if all(isinstance(item, dict) and "prompt" in item for item in data):
        return [item["prompt"] for item in data]

    # If neither of the above, raise an error:
    raise ValueError(
        "Expected JSON list of strings or list of dicts with 'prompt' field. "
        "Found something else."
    )

all_texts = load_prompts_json(PROMPT_FILE)

# If NUM_SAMPLES is specified, truncate
